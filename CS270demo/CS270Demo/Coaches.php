...
<nav id="topNav">
    ...
    <body>
        ...
        <div id="coachForm">
        <form style="color:whitesmoke;" name="coachForm" action="" method="get">
            Select a Coach to get more information on: <br><br>
           <select name="coachname">
               <?php
               $res=mysqli_query($conn,"select * from Coaches order by coachname asc");
               while($row=mysqli_fetch_array($res))
               {
               ?>
               <option><?php echo $row["CoachName"];?></option>
               <?php
               }
               ?>
               </select>
        ...
        <?php        
               if (isset($_GET['coachname'])) {
                ...
                $sql="SELECT * from Coaches where CoachName = '".$_GET['coachname']."'";
